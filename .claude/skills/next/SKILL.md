...
1. **Identificação:** Leia `docs/tasks.md`. Localize a primeira task pendente (sem `[x]`). **Pule as canceladas** — tachadas no formato `- [ ] ~~texto~~ (cancelada: motivo)` que o `/sync` aplica: continuam `[ ]`, mas não devem ser executadas.
